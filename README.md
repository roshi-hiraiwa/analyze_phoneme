...
※Google Text-to-Speech API v1beta1において、特定の単語が含まれる際に音声生成が失敗する不具合（API側のバグ）を確認したため、技術検証フェーズで開発を凍結しました。（2025年12月時点）

## 検証用の解析結果
開発段階で出力に成功した検証用の解析結果のサンプルです。<br>
ヒートマップの赤色領域(Red: Red Region)と、音声APIから取得したタイムスタンプ(Blue: Timestamp)を照合し、"ギラグレイド"から"ギラ"という音響的特徴を特定しています。
![解析結果のサンプル](target/validation/comparison_TP_ギラグレイド.png)
